...
<body>
  ...
  <div id="primary" class="single-post">
    <div class="inside">
      <div class="primary">
        <h1>Archive for the '<?php category() ?>' Category</h1>
        <ul class="dates">
          <?php 
        $catlist = list_category(true);
        foreach($catlist as $link): ?>
        <li>
          <span class="date"><?php echo date('j.m.y', $link['timestamp']) ?></span>
          <a href="<?php echo $link['url']?>"><?php echo $link['key']?></a> 
          posted in 
          <a href="<?php echo c2url() ?>" title="View all posts in <?php category() ?>" rel="category tag"><?php category() ?></a>  		 
        </li>
      <?php endforeach; ?>
    </ul>
  </div>
  ...
  <div class="secondary">
    ...
    <div class="featured">
      <p>Welcome to the archives here at <?php sitename() ?>. Have a look around.</p>
    </div>
  </div>
  <div class="clear"></div>
</div>
